          ...
          <div class="inMaskCon pa center pr"  v-show="tips1">
            ...
            <div class="container tac">
              <div>
                ...
                <div class="inContent tal pr "  v-show="isSlect2"  :class="{active: isSlect3 }">
                  <p class="f14" style="margin-top:20px;">在PUBG绝地求生中选择自建房间模式</p>
                  <div class="tips1 topCenter pa line">3</div>
                  <i class="triggle pa topCenter"></i>
                  <div class="pa selectCon f14 topCenter" @click.stop="selectFn(3)">
                    <i class="pr"><b class="f12 pa center">✔</b></i>
                    已准备
                  </div>
                </div>
                <div class="inContent tal pr" v-show="isSlect3" :class="{active: isSlect4}">
                  <p class="f14 pr">房间名：{{params.gameRoomName}}
                    <b  class="pa cp" :class="{mainCol:roomMsg == '已复制'}" style="right:10px; text-decoration:underline; color:#888888;"
                            v-clipboard:copy="params.gameRoomName"
                            v-clipboard:success="onCopy"
                            v-clipboard:error="onError"
                    >{{roomMsg}}</b></p>
                  <p class="f12">*创建游戏房间时输入</p>
                  <div class="tips1 topCenter pa line">4</div>
                  <i class="triggle pa topCenter"></i>
                  <div class="pa selectCon f14 topCenter"  @click.stop="selectFn(4)">
                    <i class="pr"><b class="f12 pa center">✔</b></i>
                    已准备
                  </div>
                </div>
                <div class="inContent tal pr" v-show="isSlect4" :class="{active: isSlect5}">
                  <p class="f14 pr">房间密码：{{params.gameRoomPwd}} 
                    <b class="pa cp"  :class="{mainCol:pwdMsg == '已复制'}" style="right:10px; text-decoration:underline; color:#888888;"
                            v-clipboard:copy="params.gameRoomPwd"
                            v-clipboard:success="onCopy2"
                            v-clipboard:error="onError2"
                    >{{pwdMsg}}</b></p>
                  <p class="f12">*创建游戏房间时输入</p>
                  <div class="tips1 topCenter pa line">5</div>
                  <i class="triggle pa topCenter"></i>
                  <div class="pa selectCon f14 topCenter"  @click.stop="selectFn(5)">
                    <i class="pr"><b class="f12 pa center">✔</b></i>
                    已准备
                  </div>
                </div>
                <div class="inContent tal pr"  v-show="isSlect5" :class="{active: isSlect6}">
                  <p class="f14 pr" >创建游戏房间</p>
                  <p class="f12">*创建房间后请于10分钟内在游戏中开始比赛，否则可能造成战绩异常。</p>
                  <div class="tips1 topCenter pa line">6</div>
                  <i class="triggle pa topCenter"></i>
                  <div class="pa selectCon f14 topCenter"  @click.stop="selectFn(6)">
                    <i class="pr"><b class="f12 pa center">✔</b></i>
                    已准备
                  </div>
                </div>
                <div class="inContent tal pr"  v-show="isSlect6" :class="{active: isSlect7}">
                  <p class="f16 margin pr">根据规则调整游戏房间设置 <b class="pa cp hide">比赛规则</b></p>
                  <div class="tips1 topCenter pa">7</div>
                  <i class="triggle pa topCenter"></i>
                  <div class="pa selectCon f14 topCenter"  @click.stop="selectFn(7)">
                    <i class="pr"><b class="f12 pa center">✔</b></i>
                    已准备
                  </div>
                </div>
    ...
    methods:{
      ...
      // 复制成功
        ...
        selectFn(i){
          ...
          let that = this
          if(i == 7){
            window.$nuxt.$store.dispatch('toClient/sendMsg',{type:10,data:{'step':i,roomId:that.params.roomId}})
            this.isShow = false
          }
        },
        num(n){
